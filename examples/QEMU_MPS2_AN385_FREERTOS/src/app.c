...
void task1_entry(void *args) {
  ...
  while (1) {
    ...
    tband_freertos_task_evtmarker(0, "PING");

    tband_freertos_task_evtmarker_begin(0, "Start 1");
    ...
    tband_freertos_task_evtmarker_begin(0, "Start 2");
    ...
    tband_freertos_task_valmarker(0, cnt);
...
void task2_entry(void *args) {
  ...
  while (1) {
    ...
    tband_evtmarker(GLOBAL_EVT_MARKER_ID, "T2 about to do something");

    tband_evtmarker_begin(GLOBAL_EVT_MARKER_ID, "T2 start something");
    xSemaphoreTake(mutex1, portMAX_DELAY);
    tband_evtmarker_end(GLOBAL_EVT_MARKER_ID);
    xSemaphoreGive(mutex1);
  }
}
...
void task3_entry(void *args) {
  ...
  while (1) {
    ...
    tband_valmarker(GLOBAL_VAL_MARKER_ID, cnt);
    cnt++;
  }
}
...
int rtos_init(void) {
  ...
  if (tband_trigger_snapshot() != 0) semihosting_exit(1);

  tband_evtmarker_name(GLOBAL_EVT_MARKER_ID, "MyEvts");
  tband_valmarker_name(GLOBAL_VAL_MARKER_ID, "MyVals");
