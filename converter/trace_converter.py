...
type TaskStateName = Literal["running", "ready", "blocked", "suspended", "deleted"]
...
type QueueKindName = Literal[
    "queue", "counting semaphore", "binary semaphore", "mutex", "recursive mutex"
]
...
class TraceState:
    ...
    stream_buffers: OrderedDict[int, StreamBuffer]
    ...
    def ensure_stream_buffer_exists(self, id: int):
    ...
    def replace_id_markers(self, s: str) -> str:
        ...
        for sb_id, sb in self.stream_buffers.items():
            if sb.name is not None:
                s = s.replace(f"#sb{sb_id}", sb.name)
            else:
                s = s.replace(f"#sb{sb_id}", str(sb_id))
...
def genevt_counter(
    name: str, property: str, task_id: int, pid: int, ts: int, value
) -> str:
    v = {
        "ph": "C",  # Counter
        "pid": pid,
...
def generate_json(state: TraceState, evts: List[TraceEvent | InvalidEvent]) -> str:
    ...
    # PID 1: Overview
    pid = 1
    trace_logs.append(genevt_metadata_pid_name("OVERVIEW: CURRENTLY RUNNING", pid))
    for task_id, task in state.tasks.items():
        ...
        for new_state in task.state_evts:
            if new_state.name == "running" and not is_running:
                trace_logs.append(
                    genevt_duration_begin(running_state_label, new_state.ts, task_id, pid, new_state.name)
                )
                is_running = True
            else:
                if is_running:
                    trace_logs.append(genevt_duration_end(new_state.ts, task_id, pid))
                    is_running = False
    ...
    for task_id, task in state.tasks.items():
        ...
        for ts, new_priority in task.priority_evts:
            trace_logs.append(
                genevt_counter(name, "priority", task_id, pid, ts, new_priority)
            )
        ...
        if len(task.priority_evts) > 0:
            ...
            trace_logs.append(
                genevt_counter(name, "priority", task_id, pid, last_ts, last_priority)
            )
    ...
    for isr_id, isr in state.isrs.items():
        ...
        for ts, evt in isr.state_evts:
            if evt == "enter":
                trace_logs.append(
                    genevt_duration_begin(name, ts, isr_id, pid, cat=name)
                )
            else:
    ...
    for q_id, q in state.queues.items():
        ...
        if q.kind == "mutex" or q.kind == "recursive mutex":
            for idx, (ts, state_evt) in enumerate(q.state_evts):
                ...
                trace_logs.append(genevt_duration_begin(state_str, ts, q_id, pid, cat="mutex"))
        else:
            for idx, (ts, state_evt) in enumerate(q.state_evts):
                trace_logs.append(genevt_counter(name, "state", q_id, pid, ts, state_evt))

            # Add final value (Prevents priorities disappearing when zooming in):
            if len(q.state_evts) > 0:
                last_state = q.state_evts[-1][1]
                trace_logs.append(genevt_counter(name, "state", q_id, pid, last_ts, last_state))

    # PID 6: Semaphores
    pid = 6
    trace_logs.append(genevt_metadata_pid_name("STREAMBUFFERS", pid))
    for sb_id, sb in state.stream_buffers.items():
        # Name:
        name = str(sb)
        trace_logs.append(genevt_metadata_task_name(name, sb_id, pid))

        for idx, (ts, state_evt) in enumerate(sb.state_evts):
            trace_logs.append(genevt_counter(name, "state", sb_id, pid, ts, state_evt))

        # Add final value (Prevents priorities disappearing when zooming in):
        if len(sb.state_evts) > 0:
            last_state = sb.state_evts[-1][1]
            trace_logs.append(genevt_counter(name, "state", sb_id, pid, last_ts, last_state))
