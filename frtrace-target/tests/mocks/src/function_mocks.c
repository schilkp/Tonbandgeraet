...
DEFINE_FAKE_VALUE_FUNC(bool, frtrace_submit_to_backend, uint8_t *, size_t, bool)
...
#if (frtrace_configUSE_BACKEND_EXTERNAL == 1)
  frtrace_submit_to_backend_reset();
  frtrace_submit_to_backend_fake.return_val = false;
#endif /* (frtrace_configUSE_BACKEND_EXTERNAL == 1) */
}
